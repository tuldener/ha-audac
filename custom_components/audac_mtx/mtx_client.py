...
class MTXClient(AudacClient):
    ...
    async def _get_all_zones_inner(self, zones_count: int, previous: dict[int, dict[str, Any]] | None = None) -> dict[int, dict[str, Any]]:
        ...
        if failed_zones:
            ...
            # handled by keeping the previous state. Only escalate to WARNING
            # when a large share of zones failed in the same poll — that hints
            # at a device- or link-wide problem, not a transient glitch.
